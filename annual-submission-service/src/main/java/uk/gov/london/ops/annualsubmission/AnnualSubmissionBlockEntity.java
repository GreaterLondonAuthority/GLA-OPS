...
public class AnnualSubmissionBlockEntity implements Serializable, LockableEntity {
    ...
    @SequenceGenerator(name = "annual_submission_block_seq_gen", sequenceName = "annual_submission_block_seq",
    ...
    @OneToMany(fetch = FetchType.LAZY, cascade = CascadeType.ALL, orphanRemoval = true,
            targetEntity = AnnualSubmissionEntryEntity.class)
    @JoinColumn(name = "block_id")
    private List<AnnualSubmissionEntryEntity> entries = new ArrayList<>();
    ...
    public AnnualSubmissionBlockEntity() {}

    public AnnualSubmissionBlockEntity(AnnualSubmissionStatusType statusType, AnnualSubmissionGrantType grantType) {
    ...
    public List<AnnualSubmissionEntryEntity> getEntries(Integer financialYear) {
        return entries.stream().filter(e -> financialYear.equals(e.getFinancialYear())).collect(Collectors.toList());
    }

    public void setEntries(List<AnnualSubmissionEntryEntity> entries) {
    ...
    public void merge(AnnualSubmissionBlockEntity updated) {
    ...
    public List<AnnualSubmissionEntryEntity> getGeneratedEntries() {
        return entries.stream()
                .filter(e -> AnnualSubmissionType.Generated.equals(e.getCategory().getType()))
                .collect(Collectors.toList());
    }

    public List<AnnualSubmissionEntryEntity> getSpentEntries() {
        return entries.stream()
                .filter(e -> AnnualSubmissionType.Spent.equals(e.getCategory().getType()))
                .collect(Collectors.toList());
    }

    public AnnualSubmissionEntryEntity getEntryByCategory(AnnualSubmissionCategory category, Integer financialYear) {
        return entries.stream()
                .filter(e -> Objects.equals(category, e.getCategory()) && Objects.equals(financialYear, e.getFinancialYear()))
    ...
    public int getTotalGenerated() {
        return getGeneratedEntries().stream().map(AnnualSubmissionEntryEntity::getValue).reduce(0, Integer::sum);
    }

    public int getTotalSpent() {
        return getSpentEntries().stream().map(AnnualSubmissionEntryEntity::getValue).reduce(0, Integer::sum);
    }

    public int getTotal(Integer financialYear) {
        return getEntries(financialYear).stream().map(AnnualSubmissionEntryEntity::getValue).reduce(0, Integer::sum);
    }
