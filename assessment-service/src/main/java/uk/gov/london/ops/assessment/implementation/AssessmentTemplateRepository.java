...
public interface AssessmentTemplateRepository extends JpaRepository<AssessmentTemplate, Integer> {
    ...
    @Query(value = "select new uk.gov.london.ops.assessment.AssessmentTemplate(at.id, at.name, at.status, "
            + "at.managingOrganisationId) from uk.gov.london.ops.assessment.AssessmentTemplate at ")
    List<AssessmentTemplate> findAllSummaries();

    @Query(value = "select * from assessment_template a where a.id in"
            + " (select assessment_template_id from programme_template_assessment_template "
            + "where programme_id = ?1 and template_id = ?2)", nativeQuery = true)
