...
@Component({
  selector: 'gla-programme-allocations-page',
  templateUrl: './programme-allocations-page.component.html',
  styleUrls: ['./programme-allocations-page.component.scss']
})
export class ProgrammeAllocationsPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    remove(this.searchOptions, {name: 'title'});
    ...
    this.searchText = projectsSearchState.organisationName || projectsSearchState.programmeName;

    // ... whereas this is used as the search text model
    this.searchTextModel = this.searchText;
    this.getProgrammeAllocations(true);
  }
  ...
  getProgrammeAllocations(showFirstPage?) {
    let projectsSearchState = this.getSearchParams();
    ...
    this.projectService.getProgrammeAllocations(
      ...
      .subscribe((rsp: any) => {
        ...
        this.programmeAllocations = rsp.content;
        this.totalItems = rsp.totalElements;

        forEach(this.programmeAllocations, (programmeAllocation) => {
          if (!programmeAllocation.fullStatus) {
            this.setFullStatus(programmeAllocation);
          }
        });
  ...
  goToProjectOverview(id) {
    this.navigationService.goToUiRouterState('project-overview', {
      projectId: id,
      backNavigation: {
        name: 'programme-allocations'
      }
    });
  }

  onAllCheckboxChange () {
    forEach(this.programmeAllocations, (project) => {
  ...
  updateAllSelectedCheckBoxState() {
    const trueCount = groupBy(this.programmeAllocations, 'isSelected').true;
    this.allSelected = trueCount && trueCount.length === this.programmeAllocations.length;
  }

  setToAssess() {
    const ids = [];
    map(this.programmeAllocations, (project) => {
  ...
  assign() {
    let selectedProjects = filter(this.programmeAllocations, {isSelected: true});
  ...
  showActions() {
    return this.canAssess || this.canTransfer;
  }

  assessEnabled() {
    if (this.canAssess) {
      return some(this.programmeAllocations, 'isSelected');
    }
    return false;
  }
