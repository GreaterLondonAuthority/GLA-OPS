...
public class AssessmentSectionSummary {
    ...
    public AssessmentSectionSummary(AssessmentTemplate assessmentTemplate, AssessmentTemplateSection sectionTemplate, List<Assessment> outcomeAssessments) {
        ...
        List<AssessmentTemplateCriteria> scoreCriteriaList = sectionTemplate.getCriteriaList().stream().filter(c -> CriteriaAnswerType.Score == c.getAnswerType()).collect(Collectors.toList());
        for (AssessmentTemplateCriteria criteriaTemplate : scoreCriteriaList) {
            criteriaList.add(new AssessmentCriteriaSummary(getTemplateMaxScore(),sectionTemplate, criteriaTemplate, outcomeAssessments));
        }
    }
    ...
    public BigDecimal getAverageScore() {
        return criteriaList.stream().map(AssessmentCriteriaSummary::getAverageScore).reduce(BigDecimal.ZERO, GlaUtils::addBigDecimals);
    }

    public BigDecimal getWeightedScore() {
        return criteriaList.stream().map(AssessmentCriteriaSummary::getWeightedScore).reduce(BigDecimal.ZERO, GlaUtils::addBigDecimals);
    }
}
