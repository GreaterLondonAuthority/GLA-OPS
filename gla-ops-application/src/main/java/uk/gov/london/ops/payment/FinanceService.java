...
import uk.gov.london.ops.project.implementation.mapper.AnnualReceiptsSummaryMapper;
...
public class FinanceService {
    ...
    private void validateLedgerRequest(ProjectLedgerItemRequest itemRequest) {
        if (ACTUAL.equals(itemRequest.getLedgerStatus())
            && !permissionService.currentUserHasPermission(PROJ_LEDGER_ACTUAL_CREATE)) {
    ...
    public AnnualSpendSummary getAnnualSpendForSpecificYear(Integer blockId, Integer year) {
        List<ProjectLedgerEntry> allByBlockIdAndYear = projectLedgerRepository.findAllByBlockIdAndFinancialYear(blockId, year);
        return annualSpendSummaryMapper.getAnnualSpendSummary(allByBlockIdAndYear, year);
    }
    ...
    public void updatePaymentEntriesManagingOrgByProgramme(int managingOrgId, int programmeId) {
        projectLedgerRepository.updatePaymentEntriesManagingOrgByProgramme(managingOrgId, programmeId);
    }
    ...
    public List<ProjectLedgerEntry> findAllBySentOnBetween(final LedgerType type,
