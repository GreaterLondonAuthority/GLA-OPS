 ...
 */
public interface ProjectLedgerRepository extends JpaRepository<ProjectLedgerEntry, Integer>,
        QuerydslPredicateExecutor<ProjectLedgerEntry> {

    String filter_active_categories_clause = " and ((ple.ledger_type not in ('PAYMENT', 'RECEIPT')) "
            + "or (ple.ledger_type = 'PAYMENT' and fc.spend_status != 'Hidden') "
            + "or (ple.ledger_type = 'RECEIPT' and fc.receipt_status != 'Hidden'))";
    ...
    ProjectLedgerEntry findFirstByBlockIdAndYearMonthAndCategoryIdAndLedgerStatus(int blockId, int yearMonth, Integer categoryId,
                                                                                  LedgerStatus ledgerStatus);

    List<ProjectLedgerEntry> findAllByBlockIdAndYearMonthAndCategoryIdAndSpendTypeAndLedgerStatus(int blockId,
                                                                                                  int yearMonth,
                                                                                                  Integer categoryId,
                                                                                                  SpendType spendType,
                                                                                                  LedgerStatus ledgerStatus);

    @Query("select  p from uk.gov.london.ops.payment.ProjectLedgerEntry p where p.blockId = ?1 and p.yearMonth >= (?2 * 100 + 4) "
            + "and p.yearMonth < ((?2 +1) * 100 + 4) and p.ledgerType = ?3")
    ...
    @Query(value = "select ple.* from project_ledger_entry ple left join finance_category fc on ple.category_id = fc.id "
            + "where ple.block_id = ?1 and ple.year_month >= (?2 * 100 + 4) and ple.year_month < ((?2 +1) * 100 + 4) "
            + filter_active_categories_clause, nativeQuery = true)
    List<ProjectLedgerEntry> findAllByBlockIdAndFinancialYear(Integer blockId, Integer year);

    @Query(value = "select ple.* from project_ledger_entry ple left join finance_category fc on ple.category_id = fc.id "
            + "where ple.block_id = ?1 and ple.year_month >= (?2 * 100 + 4) and ple.year_month < ((?3 +1) * 100 + 4) "
            + filter_active_categories_clause, nativeQuery = true)
    ...
    List<ProjectLedgerEntry> findHistoricActualsAndFutureForecasts(Integer blockId, Integer yearMonthFrom,
    ...
    List<ProjectLedgerEntry> findByProjectIdAndSubCategoryAndLedgerStatusIn(Integer projectId,
                                                                            String subCategory,
                                                                            Set<LedgerStatus> statusSet);
    ...
    default List<ProjectLedgerEntry> findAll(String projectIdOrName,
        ...
        ProjectLedgerEntryPredicateBuilder query = new ProjectLedgerEntryPredicateBuilder();
        ...
        if (predicate != null) {
            return (List<ProjectLedgerEntry>) findAll(predicate);
        } else {
            return findAll();
        }
    }

    List<ProjectLedgerEntry> findAllByLedgerTypeAndSentOnBetween(LedgerType type, OffsetDateTime from, OffsetDateTime to);
    ...
    @Query(value = "select new "
            + "uk.gov.london.ops.project.SAPMetaData(p.vendorName, p.transactionDate, p.category, p.categoryId, "
            + "p.sapCategoryCode, p.transactionNumber, p.value, p.ledgerSource, p.spendType) "
            + "from uk.gov.london.ops.payment.ProjectLedgerEntry p "
            + "where p.projectId = ?1 and p.blockId = ?2 and p.yearMonth = ?3 and p.ledgerType=?4 and p.ledgerStatus = ?5 "
            + "and p.categoryId = ?6 order by p.transactionDate")
    List<SAPMetaData> getSapMetaData(Integer projectId, Integer blockId, Integer yearMonth, LedgerType ledgerType,
                                     LedgerStatus ledgerStatus, Integer categoryId);


    @Query(value = "select distinct(fa.name) from project_ledger_entry ple "
            + "inner join funding_activity_project_ledger_entry fap on ple.id = fap.project_ledger_entry_id "
            + "inner join funding_activity fa on fa.id = fap.funding_activity_id where claim_id = ?1", nativeQuery = true)
    List<String> getAllActivitiesNameByClaimId(Integer claimId);

    @Transactional
    @Modifying
    void deleteAllByProjectId(Integer projectId);

    List<ProjectLedgerEntry> findAllByCategoryAndSubCategory(String category, String subCategory);

    @Modifying
    @Query("UPDATE uk.gov.london.ops.payment.ProjectLedgerEntry p SET p.sapVendorId = :sapVendorId where p.id = :id")
    int updateSapVendorId(@Param("sapVendorId") String sapVendorId, @Param("id") int id);
