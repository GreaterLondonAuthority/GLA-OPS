...
public class PermissionServiceImpl implements PermissionService {
    ...
    public PermissionServiceImpl(FeatureStatus featureStatus, RoleService roleService, UserServiceImpl userService,
    ...
    public Set<String> getPermissionsForUser(UserEntity user) {
        ...
        for (Role role : userApprovedRoles) {
            ...
            if (role.getName().equals("ROLE_ORG_ADMIN") && role.getOrganisation().getEntityType()
                    .equals(OrganisationType.TECHNICAL_SUPPORT.getId())) {
                permissions.add(SYS_DASHBOARD.getPermissionKey());
            }

            if (featureStatus.isEnabled(Feature.ProgrammeAllocationsPage)
                    && (role.getOrganisation().isRegulated() || role.getOrganisation().getEntityType().equals(OrganisationType.LOCAL_AUTHORITY.getId()))) {
                permissions.add(VIEW_PROGRAMME_ALLOCATIONS.getPermissionKey());
            }
        }
    ...
    private static final Set<String> ops_admin_permissions = new HashSet<>(Arrays.asList(
            ...
            ORG_EDIT_NAME.applyingFor(PERMISSION_ALL),
            ...
            USER_EDIT.getPermissionKey(),
            USERS_ASSIGN_PRIMARY.getPermissionKey(),
            USERS_ASSIGN_SIGNATORY.getPermissionKey(),

            PROG_MANAGE.getPermissionKey(),
            PROG_MANAGE_CE_CODE.getPermissionKey(),
            TEMP.getPermissionKey(),
            TEMP_MANAGE.getPermissionKey(),
            TEMP_EDIT_INTERNAL_BLOCK.getPermissionKey(),
            TEMP_EDIT_EXTERNAL_BLOCK.getPermissionKey(),

            PROJ_MILESTONE_WITHDRAW.getPermissionKey(),
            ...
            PROJ_DELETE.getPermissionKey(),
            PROJ_PAYMENT_SUSPEND.getPermissionKey(),
            ...
            CANCEL_PAYMENT_APPROVAL.getPermissionKey(),

            ADMIN_CONTENT.getPermissionKey(),
            ADMIN_SKILL_PROFILES.getPermissionKey(),
            CONTRACT_TYPES.getPermissionKey(),
            ...
            SAP_DATA_ERRORS.getPermissionKey(),

            GIVE_ACCESS_TO_ORG_VIA_TEMPLATE.getPermissionKey(),
            BROADCAST_APPROVE.getPermissionKey(),
            BLOCK_USAGE.getPermissionKey(),
            EMAIL_REPORTS.getPermissionKey(),
            SWITCH_STORAGE.getPermissionKey(),
            CLOSE_REQUEST_ORG_ADMIN.getPermissionKey()
    ));

    private static final Set<String> gla_prog_admin_permissions = new HashSet<>(Arrays.asList(
            ASSESSMENT_TEMPLATE_MANAGE.getPermissionKey(),
    ...
    private static final Set<String> gla_org_admin_permissions = new HashSet<>(Arrays.asList(
            ...
            USER_CHANGE_STATUS.getPermissionKey(),
            USERS_ASSIGN_SIGNATORY.getPermissionKey(),
            USER_EDIT_PROVIDER_ROLE.getPermissionKey(),
    ...
    private static final Set<String> additional_gla_pm_permissions = new HashSet<>(Arrays.asList(
            ...
            BROADCAST_CREATE.getPermissionKey(),
            BROADCAST_DELETE.applyingFor(MY_ORG_ID_PLACEHOLDER)
    ...
    private static final Set<String> gla_read_only_permissions = new HashSet<>(Arrays.asList(
            ...
            PROJ_VIEW_ASSIGNEE.getPermissionKey(),
            ...
            BROADCAST.getPermissionKey(),
            VIEW_PROGRAMME_ALLOCATIONS.getPermissionKey()
    ));
    ...
    private static final Set<String> additional_org_admin_permissions = new HashSet<>(Arrays.asList(
            ...
            ORG_EDIT_REGISTRATION_KEY.applyingFor(MY_ORG_ID_PLACEHOLDER),
            ORG_EDIT_TYPE.getPermissionKey(),
            ORG_EDIT_NAME.applyingFor(MY_ORG_ID_PLACEHOLDER),
            USER_PERMISSIONS.applyingFor(MY_ORG_ID_PLACEHOLDER),
            USER_REMOVE.applyingFor(MY_ORG_ID_PLACEHOLDER),
            USER_APPROVE.applyingFor(MY_ORG_ID_PLACEHOLDER),
            USER_CHANGE_STATUS.getPermissionKey(),
    ...
    private static final Set<String> only_project_editor_permissions = new HashSet<>(Arrays.asList(
            USER_REQUEST_ORG_ADMIN.getPermissionKey()
    ));
    ...
    private static final Set<String> internal_block_editor_permissions = new HashSet<>(Arrays.asList(
            ASSESSMENT_VIEW.getPermissionKey(),
            ASSESSMENT_MANAGE.getPermissionKey(),
            ASSESSMENT_TEMPLATE_MANAGE.getPermissionKey(),
            TEMP.getPermissionKey(),
    ...
    private static final Map<String, Set<String>> permissions_map = new HashMap<String, Set<String>>() {{
        ...
        put(GLA_PROGRAMME_ADMIN, gla_prog_admin_permissions);
        ...
        put(PROJECT_EDITOR, concatPermissions(projectEditor, only_project_editor_permissions));
        ...
        techAdminPermissions.add(TEMP.getPermissionKey());
        techAdminPermissions.add(TEMP_MANAGE.getPermissionKey());
        ...
        techAdminPermissions.add(BLOCK_USAGE.getPermissionKey());
        techAdminPermissions.add(SWITCH_STORAGE.getPermissionKey());
        put(TECH_ADMIN, techAdminPermissions);

        put(INTERNAL_BLOCK_EDITOR, internal_block_editor_permissions);
    }};
    ...
    public boolean userHasPermissionForOrganisation(UserEntity user, PermissionType permission, Integer orgId) {
        return userHasPermissionForOrganisation(user, permission.getPermissionKey(), orgId);
    }

    public boolean userHasPermissionForOrganisation(UserEntity user, String permission, Integer orgId) {
    ...
    private List<PermissionDetails> getPermissionDescriptionForRole(String role) {
        ...
        for (String permissionKey : PermissionServiceImpl.permissions_map.get(role)) {
            String key;
    ...
    public Map<String, List<PermissionDetails>> getPermissions() {
        return (userService.currentUser().isOpsAdmin() || userService.currentUser().isGlaOrgAdmin()
                || userService.currentUser().isSpm() || userService.currentUser().isPm()
               ) ? getPermissionDescriptionMap() : getPermissionDescriptionMapForCurrentUser();
    }
