...
class PostLoginAPI @Autowired constructor(val postLoginService: PostLoginService) {
    ...
    @RequestMapping(value = ["/legalStatus"], method = [RequestMethod.GET], produces = ["application/json"])
    @ApiOperation(value = "notify if user organisations have legal status populated")
    fun notifyMissingLegalStatus(): String? {
        return postLoginService.notifyMissingLegalStatus()
    }
}