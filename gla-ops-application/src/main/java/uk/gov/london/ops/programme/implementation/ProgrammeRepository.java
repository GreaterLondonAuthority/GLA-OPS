...
public interface ProgrammeRepository extends JpaRepository<Programme, Integer> {
    ...
    @Query(value = "select  new uk.gov.london.ops.domain.EntityCount( p.template.id, count(p)) from uk.gov.london.ops.project.Project p where p.programme.id = ?1  group by p.template.id ")
