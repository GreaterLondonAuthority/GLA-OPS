...
@JoinData(sourceTable = "project_budgets", sourceColumn = "id", targetTable = "project_block",
...
public class ProjectBudgetsBlock extends BaseFinanceBlock {
    ...
    private void generateAllYearSummaryDifferences(ProjectDifferences differences, ProjectBudgetsYearlySummary thisYearlySummary,
            ProjectBudgetsYearlySummary otherYearlySummary) {
        ...
        if (thisYearlySummary != null && otherYearlySummary != null) {
            ...
            if (thisYearSummaryRow != null && otherYearSummaryRow != null) {
                if (GlaUtils.compareBigDecimals(thisYearSummaryRow.getForecastValueTotal(),
                        otherYearSummaryRow.getForecastValueTotal()) != 0) {
                    differences.add(new ProjectDifference(thisYearSummaryRow, "forecastValueTotal"));
                }
                if (GlaUtils
                        .compareBigDecimals(thisYearSummaryRow.getActualValueTotal(), otherYearSummaryRow.getActualValueTotal())
                        != 0) {
                    differences.add(new ProjectDifference(thisYearSummaryRow, "actualValueTotal"));
                }
                if (GlaUtils.compareBigDecimals(thisYearSummaryRow.getRemainingForecastAndActualsTotal(),
                        otherYearSummaryRow.getRemainingForecastAndActualsTotal()) != 0) {
    ...
    private void generateYearlySummaryDifferences(ProjectDifferences differences, ProjectBudgetsYearlySummary thisYearlySummary,
            ProjectBudgetsYearlySummary otherYearlySummary) {
    ...
    private void compareTotals(ProjectDifferences differences, ProjectBudgetsBlock other) {
        ...
        if (thisTotals != null && otherTotals != null) {
            if (GlaUtils
                    .compareBigDecimals(thisTotals.getAvailableToForecastCapital(), otherTotals.getAvailableToForecastCapital())
                    != 0) {
                differences.add(new ProjectDifference("totals", "availableToForecastCapital"));
            }

            if (GlaUtils
                    .compareBigDecimals(thisTotals.getAvailableToForecastRevenue(), otherTotals.getAvailableToForecastRevenue())
                    != 0) {
                differences.add(new ProjectDifference("totals", "availableToForecastRevenue"));
            }

            if (GlaUtils
                    .compareBigDecimals(thisTotals.getLeftToSpendOnProjectCapital(), otherTotals.getLeftToSpendOnProjectCapital())
                    != 0) {
                differences.add(new ProjectDifference("totals", "leftToSpendOnProjectCapital"));
            }

            if (GlaUtils.compareBigDecimals(thisTotals.getApprovedProjectForecastCapital(),
                    otherTotals.getApprovedProjectForecastCapital()) != 0) {
                differences.add(new ProjectDifference("totals", "approvedProjectForecastCapital"));
            }

            if (GlaUtils.compareBigDecimals(thisTotals.getUnapprovedProjectForecastCapital(),
                    otherTotals.getUnapprovedProjectForecastCapital()) != 0) {
                differences.add(new ProjectDifference("totals", "unapprovedProjectForecastCapital"));
            }

            if (GlaUtils
                    .compareBigDecimals(thisTotals.getLeftToSpendOnProjectRevenue(), otherTotals.getLeftToSpendOnProjectRevenue())
                    != 0) {
                differences.add(new ProjectDifference("totals", "leftToSpendOnProjectRevenue"));
            }

            if (GlaUtils.compareBigDecimals(thisTotals.getApprovedProjectForecastRevenue(),
                    otherTotals.getApprovedProjectForecastRevenue()) != 0) {
                differences.add(new ProjectDifference("totals", "approvedProjectForecastRevenue"));
            }

            if (GlaUtils.compareBigDecimals(thisTotals.getUnapprovedProjectForecastRevenue(),
                    otherTotals.getUnapprovedProjectForecastRevenue()) != 0) {
