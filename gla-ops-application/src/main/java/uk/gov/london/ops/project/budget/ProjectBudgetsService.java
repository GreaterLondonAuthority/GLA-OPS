...
public class ProjectBudgetsService extends BaseProjectFinanceService implements EnrichmentRequiredListener {
    ...
    private ProjectBudgetsBlock getProjectBudgetsBlockIncludingCalculatedData(Project project,
            ProjectBudgetsBlock projectBudgetsBlock) {
        ...
        if (from != null) {
            ...
            List<ProjectLedgerEntry> entries = financeService
                    .findHistoricActualsAndFutureForecasts(projectBudgetsBlock.getId(), fromYearMonth, nowYearMonth, toYearMonth);
    ...
    public Project updateProjectBudgets(Project project, ProjectBudgetsBlock projectBudgetsBlock, boolean releaseLock) {
        ...
        for (ProjectBudgetsAttachment attachment : deletedAttachments) {
            auditService.auditCurrentUserActivity(
                    "Attachment " + attachment.getFileName() + " was deleted on project Spend block " + projectBudgetsBlock
                            .getId());
        }
        ...
        projectBudgetsBlock.getAttachments().stream().filter(attachment -> attachment.getId() == null).forEach(attachment -> {
            ...
            auditService.auditCurrentUserActivity(
                    String.format("document %s with ID %d of type %s attached with total revenue %d and total capital %d",
                            ...
                            projectBudgetsBlock.getRevenue(), projectBudgetsBlock.getCapital()));
        });
    ...
    public void enrichProject(Project project, boolean enrichmentForComparison) {
        if (enrichmentForComparison) {
            Optional<NamedProjectBlock> first = project.getProjectBlocksSorted().stream()
                    .filter(pb -> pb.getBlockType().equals(ProjectBlockType.ProjectBudgets)).findFirst();
            ...
            if (first.isPresent()) {
                ...
                List<AnnualSpendSummary> annualSpendForSpecificYears = financeService
                        .getAnnualSpendForSpecificYears(pbBlock.getId(), populatedYears);
