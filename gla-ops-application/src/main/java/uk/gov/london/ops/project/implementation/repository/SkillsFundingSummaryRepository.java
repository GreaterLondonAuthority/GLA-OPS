...
public interface SkillsFundingSummaryRepository extends JpaRepository<SkillsFundingSummaryEntity, Integer> {
    ...
    @Query(value = "select new uk.gov.london.ops.project.skills.SkillsFundingGroupedSummary(s.ukprn, s.academicYear, s.period, "
            + "s.grantType, sum(s.totalPayment)) "
    ...
    List<SkillsFundingGroupedSummary> getGroupedSummariesByUkprnAndYearAndGrantType(Integer ukprn,
                                                                                    Integer year,
    ...
    @Query(value = "select count(c.id) from claim c "
            ...
            + "where lgb.grant_type='AEB_PROCURED' and lge.type = 'DELIVERY' and lge.academic_year = ?1 and lge.period = ?2",
            nativeQuery = true)
