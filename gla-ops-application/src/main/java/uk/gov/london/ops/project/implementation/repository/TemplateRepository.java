...
public interface TemplateRepository extends JpaRepository<Template, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update template_block set detached_block_template_id = template_id, template_id = null where template_id = ?1 and display_order = ?2", nativeQuery = true)
    int copyTemplateIdToDetachedID(Integer projectId, Integer displayOrder);


    @Transactional
    @Modifying
    @Query(value = "update template_block set template_id = detached_block_template_id, detached_block_template_id =null where detached_block_template_id = ?1 and id = ?2", nativeQuery = true)
