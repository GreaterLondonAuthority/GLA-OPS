...
public class ProjectMilestonesService extends BaseProjectService implements PostCloneNotificationListener,
    ...
    public ProjectMilestonesBlock createNewMilestone(Integer projectId, Integer blockId, Milestone milestone, boolean autosave) {
        ...
        ProjectMilestonesBlock block = (ProjectMilestonesBlock) project
                .getSingleBlockByTypeAndId(ProjectBlockType.Milestones, blockId);
        checkForLock(block);
    ...
    public Project updateProjectMilestones(Project project, ProjectMilestonesBlock milestonesBlock, Integer blockId,
            boolean autosave) {
        ProjectMilestonesBlock block = (ProjectMilestonesBlock) project
                .getSingleBlockByTypeAndId(ProjectBlockType.Milestones, blockId);
        checkForLock(block);
    ...
    public Project deleteMilestoneFromProject(Project project, Milestone milestone, Integer blockId, boolean autosave) {
        ...
        if (milestone.isManuallyCreated()) {
            ...
            if (fundingBlock != null) {
                List<Integer> yearsMilestoneUsed = fundingService
                        .getYearsMilestoneUsed(project.getId(), fundingBlock.getId(), milestone.getSummary());
                if (!yearsMilestoneUsed.isEmpty()) {
                    String message = "You can't delete this milestone as payments are profiled against it in " + GlaUtils
                            .getFinancialYearList(yearsMilestoneUsed, ", ")
            ...
            auditService.auditCurrentUserActivity(
                    String.format("Removed milestone %d: %s from project with id: %d", milestone.getId(),
                            milestone.getSummary(), project.getId()));
            return project;
        } else {
    ...
    public void claim(Integer projectId, Integer milestoneId, Milestone updatedMilestone) {
        ...
        if (Template.MilestoneType.MonetaryValue.equals(project.getTemplate().getMilestoneType())) {
            // as for monetary split the amount is calculated dynamically, we don't need to validate it here
    ...
    public ProjectMilestonesBlock attachMilestoneEvidence(Integer projectId, Integer blockId, Integer milestoneId, Integer fileId,
            boolean releaseLock) {
    ...
    public ProjectMilestonesBlock removeMilestoneEvidence(Integer projectId, Integer blockId, Integer milestoneId,
            Integer attachmentId, boolean releaseLock) {
        Project project = get(projectId);
        NamedProjectBlock projectBlockById = project.getProjectBlockById(blockId);
        ...
        Optional<StandardAttachment> first = existingMilestone.getAttachments().stream()
                .filter(m -> m.getId().equals(attachmentId)).findFirst();
        if (first.isPresent()) {
            StandardAttachment attachment = first.get();
            existingMilestone.getAttachments().remove(attachment);
            auditService.auditCurrentUserActivity("Attachment " + attachment.getFileName()
                    + " was deleted from Milestone " + milestoneId + " on project: " + project.getId());
        } else {
    ...
    public PaymentGroup generatePaymentsForProject(Project project, String approvalRequestedBy) {
        ProjectMilestonesBlock milestonesBlock = project.getMilestonesBlock();
        if (milestonesBlock != null && (milestonesBlock.getApprovalWillCreatePendingPayment() || milestonesBlock
