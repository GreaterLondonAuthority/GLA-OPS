...
public class TemplateService implements CRUDServiceInterface<Integer, Template>, PortableEntityProvider {
    ...
    private final Set<ProjectBlockType> allowedBlockTypesToAdd = new HashSet<>(Arrays.asList(
            ...
            ProjectBlockType.DeliveryPartners,
            ProjectBlockType.ProjectObjectives,
            ProjectBlockType.OtherFunding,
            ProjectBlockType.UserDefinedOutput,
            ProjectBlockType.ProjectElements
    ));
    ...
    public String getTemplateAsJsonString(Template template, boolean sanitise) throws IOException {
        ...
        if (sanitise) {
            ...
            List<Class> excludedClasses = Arrays.asList(
                    Contract.class, OutputConfigurationGroup.class, FundingClaimCategory.class, MarketType.class, Question.class,
    ...
    void validateTemplate(Template template) {
        ...
        for (TemplateBlock tb : template.getBlocksEnabled()) {
            if (tb.getBlockDisplayName() != null && tb.getBlockDisplayName().length() > BLOCK_NAME_MAX_SIZE) {
                throw new ValidationException(
                        "Cannot have a block name with a name longer thant " + BLOCK_NAME_MAX_SIZE + "! (" + tb
                                .getBlockDisplayName() + ")");
            }
    ...
    public void addInternalBlock(int templateId, InternalTemplateBlock block) {
        ...
        save(template);
        InternalTemplateBlock savedBlock = template.getInternalBlockByTypeAndDisplayOrder(block.getType(), block.getDisplayOrder());
    ...
    public void updateMilestoneNaSelectable(Integer templateId, Integer blockDisplayOrder, Integer processingRouteId,
        ...
        templateProjectService
                .updateMilestoneNaSelectable(template, blockDisplayOrder, processingRouteId, milestoneId, naSelectable);
    }

    public void updateBlockDisplayName(Integer templateId, Integer blockDisplayOrder, ProjectBlockType blockType, String oldName,
            String newName) {
        Template template = find(templateId);
    ...
    public void updateCanClaimActivity(Integer templateId, Integer displayOrder, boolean canClaimActivity) {
        ...
        if (canClaimActivity && !fundingTemplateBlock.isFundingBalanceEnforced()) {
            throw new ValidationException("To enable activities claims you have to enforce funding balance first");
        }
