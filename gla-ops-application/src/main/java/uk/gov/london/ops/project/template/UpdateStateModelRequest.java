...
public class UpdateStateModelRequest {
    ...
    public ProjectState getToState(ProjectState fromState) {
        return stateUpdates.stream().filter(su -> Objects.equals(su.getFrom().getStatus(), fromState.getStatus())
                && Objects.equals(su.getFrom().getSubStatus(), fromState.getSubStatus()))
