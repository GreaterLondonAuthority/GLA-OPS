...
@JoinData(sourceTable = "unit_details_block", sourceColumn = "id", targetTable = "project_block",
...
public class UnitDetailsBlock extends NamedProjectBlock {
    ...
    private boolean unitsByNumberOfPeopleValid() {
        return getTotalUnits()
                .equals(nullSafeAdd(type1Units, type2Units, type3Units, type4Units, type5Units, type6Units, type7Units,
                        type8Units));
    }
    ...
    public TenureProfiles getTenureProfiles() {
        ...
        for (Map.Entry<Integer, Integer> result : totalUnitsByExternalId.entrySet()) {
            ...
            if (tenureType != null) {
                TenureProfile profile = new TenureProfile(tenureType.getExternalId(), tenureType.getName(), profiledUnits,
                        result.getValue());
        ...
        List<TenureProfile> breakdown = profiles.stream().sorted(Comparator.comparingInt(TenureProfile::getExtId))
    ...
    private void compareUnitsTables(ProjectDifferences differences, Map<Integer, UnitDetailsTableEntry> left,
            Map<Integer, UnitDetailsTableEntry> right) {
    ...
    private Map<Integer, UnitDetailsTableEntry> getEntriesFromList(Set<UnitDetailsTableEntry> entries, boolean rentals,
            boolean sales) {
