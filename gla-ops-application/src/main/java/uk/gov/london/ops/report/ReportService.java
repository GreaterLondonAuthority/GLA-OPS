...
public class ReportService {
    ...
    @Value("#{new Double('${max.adhoc.sql.cost}')}")
    ...
    public List<Project> getProjects(Integer programmeId, Integer projectType, String projectStatus, String borough) {
        ...
        if (projectType != null) {
            projects = projectService.findAllByProgrammeAndTemplate(programme, new Template(projectType, ""));
        } else {
            projects = projectService.findAllByProgramme(programme);
        }
    ...
    private void validateCost(Double queryCost, Double maxAllowedCost, String auditMessage, String errorMessage ) {
        if (queryCost > maxAllowedCost) {
            auditService.auditActivityForUserNewTransaction(userService.currentUser().getUsername(), auditMessage);
        ...
        if (enforceReportCost || fakeCostExceeded) {
            validateCost(queryCost,
                    ...
                    String.format("Report %s was too expensive to run: Query: %.2f Limit: %.2f Filters Selected: %s",
                                  ...
                                  "The dataset you have chosen is too large. There is too much information in your report. " +
                                   "Try changing the filters to reduce the information - for example you may have selected too many programmes.");
        }
        return queryCost;
    }
