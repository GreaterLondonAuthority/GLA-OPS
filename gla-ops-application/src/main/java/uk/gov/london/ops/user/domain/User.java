...
public class User extends BaseUser implements UserDetails, Serializable, NotificationTargetEntity {
    ...
    public Set<Organisation> getManagingOrganisations() {
        Set<Organisation> managingOrganisations = new HashSet<>();
        for (Organisation organisation : getOrganisations()) {
            managingOrganisations.add(organisation.isManagingOrganisation() ? organisation : organisation.getManagingOrganisation());
        }
        return managingOrganisations;
    }

    public Set<Integer> getManagingOrganisationsIds() {
        return getManagingOrganisations().stream().map(Organisation::getId).collect(Collectors.toSet());
    }
