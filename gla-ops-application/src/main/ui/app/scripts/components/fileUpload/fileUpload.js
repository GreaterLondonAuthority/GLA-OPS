  ...
  handleSelected(e) {
    ...
    if (exceedLimit.length) {
      ...
      const msg = {
        ...
        copy: this.maxCombinedUploadSizeInMb ? `Combined file size exceeds the combined file size limit (${this.maxCombinedUploadSizeInMb}MB)`
          : `The file ${exceedNames.toString()} exceeds the supported size of ${this.MAX_FILE_SIZE_MO}MB. Add a smaller document.`
      }
      this._onError(msg);
    } else {
...
angular.module('GLA')
  .component('fileUpload', {
    bindings: {
      ...
      componentInputId: '<',
      maxFileSizeInMb: '<',
      maxCombinedUploadSizeInMb: '<',
      remainingCombinedFileSize: '<',
      postUrl: '<'
    },
