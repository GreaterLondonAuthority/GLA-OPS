...
class PaymentSummaryCtrl {
  constructor($stateParams, UserService, ReclaimModal, ResendModal, PaymentService, $state, dateFilter, numberFilter, currencyFilter, ConfirmationDialog, ToastrUtil, ErrorService, $window) {
  ...
  resendPayments() {
    ...
    modal.result.then((response) => {
      return this.PaymentService.resend(this.payment.id, response.wbsCode, response.ceCode, response.sapVendorId, response.companyName);
    }).then((data) => {
...
PaymentSummaryCtrl.$inject = ['$stateParams', 'UserService', 'ReclaimModal', 'ResendModal', 'PaymentService', '$state', 'dateFilter', 'numberFilter', 'currencyFilter', 'ConfirmationDialog', 'ToastrUtil', 'ErrorService', '$window'];
