...
function TransitionService(UserService) {
  ...
  return {
    ...
    getMenuItems(project, featureToggles) {
      ...
      if (featureToggles.isProjectSharingEnabled && UserService.hasPermission('proj.share')) {
        menuItems.push({
          menuItem: true,
          type: 'action',
      ...
      if (featureToggles.isAllowAllFileDownloadEnabled && UserService.hasPermission('proj.download.zip')) {
        menuItems.push({
          menuItem: true,
          type: 'action',
