...
RegistrationTypeCtrl.$inject = ['$scope', '$log', 'OrganisationService', 'UserService', '$state', 'SessionService', 'allowNewRegistrationProcess'];

function RegistrationTypeCtrl($scope, $log, OrganisationService, UserService, $state, SessionService, allowNewRegistrationProcess) {
  var ctrl = this;

  ctrl.done = false;
  ctrl.allowNewRegistrationProcess = allowNewRegistrationProcess;
  ...
  ctrl.registerNewOrg = function() {
    SessionService.setOrgRegistration(null);
    $state.go('organisation.registration-programme');
  };
