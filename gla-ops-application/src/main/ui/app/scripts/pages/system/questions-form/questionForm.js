...
class QuestionForm {
  ...
  $onInit() {
    ...
    if (this.question.answerType === 'FileUpload') {
      this.fileUploadLimitType = this.question.maxCombinedUploadSizeInMb ? 'CombinedFileSize' :
        this.question.maxUploadSizeInMb ? 'NumberOfFiles' : null;
    }
  }

  fileLimitTypeChanged() {
    if (this.fileUploadLimitType === 'NumberOfFiles') {
      this.question.maxCombinedUploadSizeInMb = null;
    }
  }
  ...
  isQuestionValid(question){
    ...
    this.hasAnswersRestrictedText = false;
    ...
    if (question.answerType === 'Dropdown') {
      let optionsAmount = this.getValidOptions(question).length;
      if(optionsAmount < 2) {
        this.hasDuplicate = true;
        return false;
      }

      if(!question.maxAnswers || optionsAmount < question.maxAnswers){
        return false;
      }

      if(question.maxAnswers > 1 && (question.answerOptions || []).some(ao => ao.option.indexOf(';') !== -1)){
        this.hasAnswersRestrictedText = true;
        return false;
      }
    }
