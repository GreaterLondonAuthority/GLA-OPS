...
function AnnualSubmissionService($http, config, $rootScope, fYearFilter, currencyFilter) {
  ...
  return {
    ...
    getTileStatus(financialYear, annualSubmissions) {
      let annualSubmission = annualSubmissions.find(obj => {
        if (obj.financialYear == financialYear) {
          return obj;
        }
      });

      if (annualSubmission === undefined) {
        return null;
      } else {
        return annualSubmission.status;
      }
    },

    getBalanceTiles(block, annualSubmissions) {
      let format = (val) => {
      ...
      return (block.totals || []).map(t => {
        return {
          ...
          status: this.getTileStatus(t.financialYear, annualSubmissions),
          items: [{
