...
function ProjectFundingService($http, config) {
  ...
  return {
    ...
    claim(projectId, blockId, financialYear, quarter, activityId) {
      if (activityId) {
        return $http.post(`${config.basePath}/projects/${projectId}/funding/${blockId}/claim/${financialYear}/quarter/${quarter}?activityIds=${activityId}`);
      }
      else {
