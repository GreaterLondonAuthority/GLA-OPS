...
public interface OutputCategoryConfigurationSummaryRepository extends JpaRepository<OutputCategoryConfigurationSummary, Integer> {
    ...
    @Query(value = "SELECT id, category, subcategory, value_type, tenure_type_id, hidden, "
            + "string_agg(CAST(output_group_id AS text), ', ') output_group_ids FROM ( "
            ...
            + "OUTPUT_CAT_CONFIG.hidden hidden, OUTPUT_GROUP_OUTPUT_CONFIG.OUTPUT_GROUP_ID output_group_id FROM OUTPUT_CAT_CONFIG "
