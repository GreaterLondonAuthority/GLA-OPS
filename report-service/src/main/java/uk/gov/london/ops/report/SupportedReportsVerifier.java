...
public class SupportedReportsVerifier extends SimpleFileVisitor<Path> {
    ...
    public SupportedReportsVerifier(JdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    private void testSqlFile(Path file) {
        ...
        testSqlFile(file, sql);
    }

    void testSqlFile(Path file, String sql) {
        try {
            jdbc.execute(sql);
    ...
    private void handleError(Path file, DataAccessException e) {
     ...
     */
    String sqlErrorSummary(DataAccessException exception) {
