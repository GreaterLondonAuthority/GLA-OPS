...
public class AnnualSubmissionService implements LockableEntityService<AnnualSubmissionBlock> {
    ...
    public List<Integer> getAvailableYearsForCreation(Integer organisationId) {
        ...
        for (Integer year = annualSubmissionsFirstYear; year < (Year.now().getValue() + annualSubmissionsNbFutureYears); year++) {
    ...
    void calculateAllowedTransitions(AnnualSubmission annualSubmission) {
        ...
        if (annualSubmission.getStatus().equals(Draft) && annualSubmission.isComplete() && !annualSubmission.anyBlocksLocked() && !currentUser.isReadOnly(annualSubmission.getOrganisationId())) {
        ...
        else if (annualSubmission.getStatus().equals(Submitted) && permissionService.currentUserHasPermissionForOrganisation(ANNUAL_SUBMISSION_REVERT.getPermissionKey(), annualSubmission.getOrganisationId())) {
            ...
            if (allPreviousYearsApproved(annualSubmission.getOrganisationId(), annualSubmission.getFinancialYear())) {
                annualSubmission.getAllowedTransitions().add(new AnnualSubmissionTransition(Approved, true));
            }
            else {
                annualSubmission.getMessages().add(new ApiErrorItem("A return for an earlier year is awaiting submission. You must approve earlier returns before you can approve this return"));
            }
        }
        else if (annualSubmission.getStatus().equals(Approved) && permissionService.currentUserHasPermissionForOrganisation(ANNUAL_SUBMISSION_REVERT.getPermissionKey(), annualSubmission.getOrganisationId())) {
            if (currentUser.isOpsAdmin() || annualSubmission.getFinancialYear() >= financialCalendar.currentYear() - 1) {
    ...
    private boolean allPreviousYearsApproved(Integer organisationId, Integer financialYear) {
        List<AnnualSubmission> submissions = annualSubmissionRepository.findAllByOrganisationIdAndFinancialYearLessThan(organisationId, financialYear);
        return submissions.isEmpty() || submissions.stream().allMatch(s -> Approved.equals(s.getStatus()));
    }

    private void calculateAllowedActions(AnnualSubmission annualSubmission, AnnualSubmissionBlock block) {
        if (permissionService.currentUserHasPermissionForOrganisation(ANNUAL_SUBMISSION_CREATE.getPermissionKey(),annualSubmission.getOrganisationId()) ) {
    ...
    public void updateStatus(Integer submissionId, AnnualSubmissionStatus status,
        String agreementText) { AnnualSubmission annualSubmission = get(submissionId);
        ...
        if (status != null && !status.equals(annualSubmission.getStatus())) {
            ...
            Map<String, Object> model = new HashMap<String, Object>() {{
                put("financialyear", financialCalendar.financialYearString(Integer.valueOf(annualSubmission.getFinancialYear())));
            ...
            if (Draft.equals(annualSubmission.getStatus()) && Submitted.equals(status)) {
                ...
                annualSubmission.setSubmittedBy(userService.currentUser());
    ...
    public AnnualSubmissionBlock getBlock(Integer submissionId, Integer blockId) {
        ...
        if (Actual.equals(statusType)) {
            ...
            if (blockYearMinus1 != null) {
                block.setOpeningBalance(blockYearMinus1.getClosingBalance());
            }
    ...
    private void calculateOpeningBalance(Integer organisationId, Integer financialYear, AnnualSubmissionBlock block) {
        if (block != null) {
            ...
            for (int i = 0; i < blocks.size() - 1; i++) {
                blocks.get(i+1).setOpeningBalance(blocks.get(i).getClosingBalance());
            }
            AnnualSubmissionBlock previousYearBlock = blocks.isEmpty() ? null : blocks.get(blocks.size() - 1);
            if (previousYearBlock != null) {
                block.setOpeningBalance(previousYearBlock.getClosingBalance());
            }
        }
    }
    ...
    public Set<AnnualSubmission> findAllAnnualSubmissionsRequiringDPF() {
        return annualSubmissionRepository.findAllMissingDPFSubmissionsUpTo(annualSubmissionsDpfLastYear);
    }
