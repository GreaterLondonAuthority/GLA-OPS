...
public class ProgressUpdateBlock extends NamedProjectBlock {
    ...
    public boolean isComplete() {
        ...
        if (StringUtils.isEmpty(progressUpdate)) return false;

        if (this.getProject().isMarkedForCorporate()) {
            if (StringUtils.isEmpty(publicUpdate)) return false;
            if (StringUtils.isEmpty(privateUpdate)) return false;
        }
