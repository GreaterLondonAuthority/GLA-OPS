...
public class FundingSectionTotalsWrapper {
    ...
    private Map<Integer, BigDecimal> capitalTotals = new HashMap<>();
    private Map<Integer, BigDecimal> revenueTotals = new HashMap<>();
    ...
    protected void addActivity(FundingActivity activity) {
        ...
        capitalTotals.put(activity.getQuarter(), addBigDecimals(capitalTotals.get(activity.getQuarter()), activity.getTotalCapitalValue()));

        if (!revenueTotals.containsKey(activity.getQuarter())) {
            revenueTotals.put(activity.getQuarter(), null);
        }

        revenueTotals.put(activity.getQuarter(), addBigDecimals(revenueTotals.get(activity.getQuarter()), activity.getTotalRevenueValue()));
    }
