...
public class FundingSummary extends FundingTotalsWrapper {
    ...
    private Map<Integer, FundingSummaryYear> fundingSummaryYears = new HashMap<>();

    public List<FundingSummaryYear> getYearBreakdown() {
        return fundingSummaryYears.values().stream().sorted(Comparator.comparing(FundingSummaryYear::getYear)).collect(Collectors.toList());
    }
