...
public class FundingYearBreakdown extends FundingTotalsWrapper {
    ...
    private Map<Integer, FundingSection> sections = new HashMap<>();
    ...
    public List<FundingSection> getSections() {
        return sections.values().stream().sorted(Comparator.comparing(FundingSection::getSectionNumber)).collect(Collectors.toList());
    }
