...
public class OrganisationMapper {
    ...
    //"Area Lead","Team","Org.Name","Active","IMS No.","Gaps","SAP Vendor No","Entity Type","Email",
    // "Address1","Add2","Add3","Add4","Add5","Postcode",
    // "CEO Title","CEO Name","Regulated?","Viability Score","Gov. Score","ID no.","Status",
    // "notes",2,3,4,5,6,7,8,9,1,1,3
    ...
    public List<Organisation> toEntities(InputStream csvInputStream, BulkUploadSummary summary) throws IOException {
        List<CSVRecord> records = CSVFormat.DEFAULT.withNullString("").withHeader(HEADERS).withSkipHeaderRecord(true)
                .withTrim().parse(new InputStreamReader(csvInputStream)).getRecords();
...
//                if (StringUtils.isNotEmpty(record.get(ENTITY_TYPE))) {
//                    organisation.setEntityType(Integer.parseInt(record.get(ENTITY_TYPE)));
//                }
    ...
    private String getScore(CSVRecord record, String column) {
        ...
        if (score == null || score.length() != 2 || ! (score.startsWith("V") || score.startsWith("G"))) {
            ...
            if (score.length() > 2) {
                log.warn("Truncating score {} to 2 characters", score);
    ...
    public List<OrganisationModel> toModel(Collection<Organisation> organisations) {
        return organisations.stream().map(OrganisationModel::from).collect(Collectors.toList());
    }
