...
public class UserMapper {
    ...
    public static UserModel toModel(User user) {
        ...
        for (Organisation organisation: user.getOrganisations()) {
            model.getOrganisations().add(OrganisationModel.from(organisation));
        }

        String primaryRole = null;
        for (Role role: user.getRoles()) {
            RoleModel roleModel = toModel(role);
            if (Role.OPS_ADMIN.equals(role.getName())) {
    ...
    public static List<UserModel> mapToModel(Collection<User> users) {
        if (users == null) {
            return null;
        }
        else {
            return users.stream().map(UserMapper::toModel).collect(Collectors.toCollection(LinkedList::new));
        }
    }
