...
public class EmailService implements InfoContributor {
    ...
    @Autowired FeatureStatus featureStatus;
    ...
    public void sendEmail(NotificationType notificationType, User user, Organisation organisation) {
        try {
            ...
            throw new RuntimeException("Unable to send notification email.", e);
        }
    }

    public void sendRegistrationRequestEmail(NotificationType notificationType, User requester, User recipient, Organisation organisation) {
        try {
            ...
            throw new RuntimeException("Unable to send registration request email.", e);
        }
    }

    public void sendUserRejectionEmail(User user, Organisation organisation) {
        try {
    ...
    String generateBody(String templateName, User recipient,  User user, Object model) throws IOException {
    ...
    private Email saveEmail(String recipient, String subject, String body, String fileName, String filePayload) {
        ...
        if (filePayload != null) {
            ...
            attachmentFile.setFileContent(filePayload.getBytes(UTF_8));
            fileService.save(attachmentFile);
                    ...
                    for (Email email : emailsToBeSent) {
                        try {
                            ...
                            if(featureStatus.isEnabled(Feature.EmailSending)) {
                                ...
                                log.debug("Email {} not sent as feature turned off.", email.getId());
                            }
    ...
    public MimeMessage toJavaMail(Email email, Session session) throws IOException, MessagingException {
        ...
        if (email.getEmailAttachments() != null && !email.getEmailAttachments().isEmpty()) {
            ...
            byte[] fileContent = fileService.getFileContent(attachment.getId());
            if (fileContent != null) {
