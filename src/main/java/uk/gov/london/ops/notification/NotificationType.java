...
import static uk.gov.london.ops.domain.EntityType.annualSubmission;
import static uk.gov.london.ops.domain.EntityType.organisation;
import static uk.gov.london.ops.domain.EntityType.payment;
import static uk.gov.london.ops.domain.EntityType.paymentGroup;
import static uk.gov.london.ops.domain.EntityType.project;
import static uk.gov.london.ops.domain.EntityType.user;
...
public enum NotificationType {
    ...
    OrganisationApproval(
            ...
            "Your {{name}} registration for {{managingOrganisationName}} has been approved and you are now the admin. Give users this code to register their account with this organisation: {{id}}. You must approve their request on the all users page.",
            organisation,
            Info,
    ...
    AnnualSubmissionAwaitingApproval(
            ...
            "An annual recoverable grant submission for {{financialyear}} has been made by {{organisation.name}}{{rollover}}. This submission is awaiting approval.",
            annualSubmission,
            Info,
            null,
            null,
    ...
    PendingSpendAuthorityThresholdApproval(
              ...
              "Your spend authority threshold has been set at £{{approvedThreshold}} for {{organisation.name}}. You can now authorise payments up to this amount.",
              user,
              Action,
              null,
              null,
    ...
    ProjectTransfer(
            ...
            "Project {{id}} has been transferred from {{fromOrganisation.name}} to {{toOrganisation.name}}. {{fromOrganisation.name}} no longer has access to this project. If you have any issues, email the OPS team at ops@london.gov.uk.",
            project,
            Info,
            null,
            null,
    ...
    ApprovedForNextStage(
            ...
            "P{{id}} for {{organisation.name}} has been approved for {{fromStatus}} and is now ready for {{toStatus}}. There may be additional information to complete on this project.",
            project,
            Info,
            null,
            null,
    ...
    NotificationType(String trigger, String text, EntityType entityType, NotificationSubType subType, String emailSubject, String emailTemplate, Collection<String> rolesNotified) {
        this(trigger, text, entityType,subType, emailSubject, emailTemplate, rolesNotified.toArray(new String[rolesNotified.size()]));
    }

    NotificationType(String trigger, String text, EntityType entityType, NotificationSubType subType, String emailSubject, String emailTemplate, String ... rolesNotified) {
