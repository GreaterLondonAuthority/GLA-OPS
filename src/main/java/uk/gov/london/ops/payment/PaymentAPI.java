...
public class PaymentAPI {
    ...
    public Page<PaymentSummary> getAll(
            ...
            @RequestParam(required = false) List<PaymentSource> paymentSources,
        ...
        boolean noSourceSpecified = paymentSources == null || paymentSources.isEmpty();

        if (noSourceSpecified) {
            paymentSources = LedgerTypeFilterOption.ALL.getRelevantSourcesAsList();
        }
...
//        }
    ...
    public  ProjectLedgerEntry reprocessPayment(@PathVariable Integer id, @RequestParam(required = false) String wbsCode) {
        return authorisedPaymentsProcessor.reprocessAuthorisedPayment(id, wbsCode);
    }
    ...
    @Secured({OPS_ADMIN})
    @RequestMapping(value = "/payments/{projectLedgerEntryId}/xmlFile", method = RequestMethod.GET, produces = "application/xml; text/xml")
