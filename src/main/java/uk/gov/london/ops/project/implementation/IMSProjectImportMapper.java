...
public class IMSProjectImportMapper {
    ...
    public static final String SOS_PAYMENT_RECLAIM = "Start On Site Payment Reclaim";
    ...
    public static final String LAND_ACQUIRED_PAYMENT = "Land Acquired Payment";
    public static final String START_ON_SITE_PAYMENT = "Start On Site Payment";

    public static final String LAND_ACQUIRED_DATE = "Land Acquired Date";
    public static final String LAND_ACQUIRED_GRANT = "Land Acquired Grant %";
    public static final String LAND_ACQUIRED_STATUS = "Land Acquired Status";
    public static final String LAND_ACQUIRED_CLAIM_STATUS = "Land Acquired Claim Status";
    ...
    public static final String MILESTONE_FOR_NON_GRANT_PAYMENT = "Milestone For Non Grant Payment";
    ...
    public static final int LAND_ACQUIRED = 3005;
    ...
    public void mapIMSUnitDetails(UnitDetailsBlock unitDetailsBlock  , CSVFile csvFile) throws ValidationException {
        ...
        CategoryValue numberOfBedrooms = refDataService.findByCategoryAndDisplayValue(CategoryValue.Category.Bedrooms, csvFile.getString(DETAILS_NUMBER_OF_BEDROOMS));
        ...
        CategoryValue unitType = refDataService.findByCategoryAndDisplayValue(CategoryValue.Category.UnitTypes, csvFile.getString(DETAILS_UNIT_TYPE));
        ...
        TemplateTenureType templateTenureType = null;
        TenureType type = null;
        String tenureString = csvFile.getString(DETAILS_TENURE_TYPE);
        for (TemplateTenureType templateTenure : template.getTenureTypes()) {
            if (templateTenure.getName().equals(tenureString)) {
                templateTenureType = templateTenure;
                type = templateTenure.getTenureType();
            }
        }
        ...
        if (templateTenureType.getMarketTypes().get(0).getAvailableForSales()) {
    ...
    public void mapProjectQuestions(Project project , CSVFile csvFile) {
        ...
        for (NamedProjectBlock namedProjectBlock : blocksByType) {
            ...
            for (TemplateQuestion templateQuestion : questionsBlock.getTemplateQuestions()) {
                ...
                if (!StringUtils.isEmpty(answer)) {
                    questionsBlock.setLastModified(environment.now());
                    questionsBlock.setModifiedBy(userService.currentUsername());
                    Answer answerByQuestionId = questionsBlock.getAnswerByQuestionId(templateQuestion.getQuestion().getId());
                    if (answerByQuestionId != null) {
                        if (templateQuestion.getQuestion().getAnswerType().equals(AnswerType.Number)) {
                            answerByQuestionId.setNumericAnswer(Double.parseDouble(answer));

                        } else {
                            answerByQuestionId.setAnswer(answer);
                        }
                    } else {
    ...
    private void mapNegotiatedGrant(Project project , CSVFile csvFile) {
        ...
        Set<ProjectTenureDetails> projectTenureDetailsEntries = negotiatedGrantBlock.getTenureTypeAndUnitsEntries();
        ProjectTenureDetails toUpdate = null;
        for (ProjectTenureDetails projectTenureDetailsEntry : projectTenureDetailsEntries) {
            if (projectTenureDetailsEntry.getTenureType().getName().equals(csvFile.getString(TENURE_TYPE))) {

                 toUpdate = projectTenureDetailsEntry;
           }
        }
        ...
        for (Milestone milestone : milestonesBlock.getMilestones()) {
            ...
            switch (milestone.getExternalId()) {
                ...
                case INTERIM:
                    ...
                    break;
                case LAND_ACQUIRED:
                    if (csvFile.getHeaders().contains(LAND_ACQUIRED_DATE)) {
                        LocalDate acquiredDate = csvFile.getDate(LAND_ACQUIRED_DATE, DATE_FORMAT);
                        if (acquiredDate !=null) {
                            milestone.setMilestoneDate(acquiredDate);
                            milestone.setMonetarySplit(csvFile.getIntegerOrNull(LAND_ACQUIRED_GRANT));
                            milestone.setMilestoneStatus(getMilestoneStatus(csvFile, LAND_ACQUIRED_STATUS));
                            milestone.setClaimStatus(getMilestoneClaimStatus(csvFile, LAND_ACQUIRED_CLAIM_STATUS));
                            handlePaymentsForMilestone(project, csvFile, milestone);
                        }
                    }
                    break;
    ...
    private void handlePaymentsForMilestone(Project project, CSVFile csvFile, Milestone milestone) {
        ...
        Integer milestonePaymentId  = csvFile.getIntegerOrNull(MILESTONE_FOR_NON_GRANT_PAYMENT);
        if (milestonePaymentId == null) {
            milestonePaymentId = COMPLETION;
        }
        ...
        boolean landAcquired = milestone.getExternalId().equals(LAND_ACQUIRED);
        ...
        } else if (landAcquired) {
            columnName = LAND_ACQUIRED_PAYMENT;
        }
        ...
        if (milestone.getExternalId().equals(milestonePaymentId)) {
        ...
        if (!pg.getLedgerEntries().isEmpty()) {
            ...
            paymentService.save(pg);
        }



        BigDecimal sosReclaim = csvFile.getCurrencyValue(SOS_PAYMENT_RECLAIM);
        ...
        if (interimReclaim != null || completionPayment != null || sosReclaim != null) {

            LocalDate reclaimDate = csvFile.getDate(RECLAIM_DATE, DATE_FORMAT);
//            MilestoneStatus reclaimStatus = getMilestoneStatus(csvFile, RECLAIM_STATUS); // commenting out as unused
            ...
            if (interim && interimReclaim != null) {
                ...
                milestone.setReclaimedGrant(interimReclaim.longValue());
                pg.getLedgerEntries().add(createReclaim(project, interimReclaim, reclaimDate, "Reclaimed Interim payment", auth, grantPayment));
            }

            if (startOnSite && sosReclaim != null) {
                milestone.setReclaimed(true);
                milestone.setReclaimedGrant(sosReclaim.longValue());
                pg.getLedgerEntries().add(createReclaim(project, sosReclaim, reclaimDate, "Reclaimed Start On Site", auth, grantPayment));
            }

            if (completion && completionPayment != null) {
                milestone.setReclaimed(true);
                milestone.setReclaimedGrant(completionPayment.longValue());
                pg.getLedgerEntries().add(createReclaim(project, completionPayment, reclaimDate, "Reclaimed Completion", auth, grantPayment));
    ...
    private ProjectLedgerEntry createPayment(Project project, Milestone milestone, BigDecimal currencyValue, LedgerType type) {
        ...
        long rounded = currencyValue.setScale(0, BigDecimal.ROUND_HALF_UP).longValue();
        switch (type) {
            case PAYMENT:
    ...
    private ProjectLedgerEntry populatePayment(ProjectLedgerEntry entry, Project project, LedgerType type) {
        ...
        if (project.getOrganisationGroupId() != null) {
            Organisation devOrg = organisationRepository.getOne(project.getDetailsBlock().getDevelopingOrganisationId());
        ...
        entry.setWbsCode(project.getProgramme().getWbsCodeForTemplate(project.getTemplateId()));
    ...
    public void handleUpdatesToGrantSource(Project project, CSVFile csvFile)  {
        ...
        ProjectTenureDetails entry = baseGrantBlock.getTenureTypeAndUnitsEntry(tenure);
        if (entry == null) {
