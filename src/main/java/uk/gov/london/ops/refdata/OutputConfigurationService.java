...
public class OutputConfigurationService  {
    ...
    public List<OutputCategoryConfiguration> getAllOutputCategories() {
        return outputCategoryConfigurationRepository.findAll();
    }
    ...
    private void validateOutputCategoryConfigurationForSave(OutputCategoryConfiguration existing, OutputCategoryConfiguration categoryConfiguration) {
    ...
    public OutputConfigurationGroup createOutputConfigurationGroup(OutputConfigurationGroup group) {
        ...
        if (group.getCategoryIDs() != null) {
            ...
            for (Integer id : group.getCategoryIDs()) {
                OutputCategoryConfiguration cat = outputCategoryConfigurationRepository.findById(id).orElse(null);
                if (cat == null) {
        ...
        if (group.getOutputTypeKeys() != null) {
            ...
            for (String key : group.getOutputTypeKeys()) {
                OutputType outputType = outputTypeRepository.findById(key).orElse(null);
                if (outputType == null) {
    ...
    public OutputConfigurationGroup addCategoryToOutputGroup(Integer groupId, Integer categoryId) {
        if (groupId == null || categoryId == null) {
            throw new ValidationException("Unable to add category to output group without group and category ids being specified");
        }
        ...
        if (cat == null) {
            throw new ValidationException(String.format("Unable to find output category with ID: %d, please create it first", categoryId));
        }
    ...
    public void updateOutputCategoryName(Integer id, String oldName, String newName) {
        ...
        templateService.updateAssessmentsAffectedByCategoryChange(one, oldName, newName);
    }
    ...
    public OutputCategoryConfiguration findByCategoryAndSubcategory(String category, String subCategory) {
        return outputCategoryConfigurationRepository.findByCategoryAndSubcategory(GlaUtils.superTrim(category), GlaUtils.superTrim(subCategory));
    }
