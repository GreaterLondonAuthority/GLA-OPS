...
public class AffordableHousingReportMapper {
    ...
    public static final Map<Integer, String> milestones = new HashMap<Integer, String>() {{
        put(Milestone.ACQUISITION_ID, ACQUISITION);
        put(Milestone.START_ON_SITE_ID, SOS);
        put(Milestone.COMPLETION_ID, COMPLETION);
    }};

    public static final Set<String> base_headers = new LinkedHashSet<>(Arrays.asList(
            GLA_OPS_PROJECT_ID,
            PROJECT_ADDRESS,
            PROJECT_TITLE,
            PROJECT_STATUS,
    ...
    private void addBlockVersionSpecificData(Project project, Map<String, Object> map, OffsetDateTime approvalDate, Integer milestoneId) {
        ...
        ProjectTenureDetails larEntry = getTenureAndUnitEntry(baseGrantBlock, TenureType.LONDON_AFFORDABLE_RENT);
        ProjectTenureDetails llrEntry = getTenureAndUnitEntry(baseGrantBlock, TenureType.LONDON_LIVING_RENT);
        ProjectTenureDetails lsoEntry = getTenureAndUnitEntry(baseGrantBlock, TenureType.LONDON_SHARED_OWNERSHIP);
        ProjectTenureDetails otherAffEntry = getTenureAndUnitEntry(baseGrantBlock, TenureType.OTHER_AFFORDABLE);
    ...
    private ProjectTenureDetails getTenureAndUnitEntry(BaseGrantBlock grantBlock, String tenureType) {
        ProjectTenureDetails entry = null;
        if (grantBlock != null) {
            entry = grantBlock.getTenureTypeAndUnitsEntry(tenureType);
        }
        return entry != null ? entry : new ProjectTenureDetails();
    }
