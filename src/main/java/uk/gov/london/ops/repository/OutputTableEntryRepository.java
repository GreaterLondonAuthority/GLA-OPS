...
public interface OutputTableEntryRepository extends JpaRepository<OutputTableEntry, Integer> {
    ...
    @Query("select  p from uk.gov.london.ops.domain.project.OutputTableEntry p where " +
    ...
    @Query("select  p from uk.gov.london.ops.domain.project.OutputTableEntry p where p.blockId = ?1 and p.config.id = ?2 and p.yearMonth = 0")
