...
public interface SkillsFundingSummaryRepository extends JpaRepository<SkillsFundingSummaryEntity, Integer> {
    ...
    @Query(value = "select new uk.gov.london.ops.domain.skills.SkillsFundingGroupedSummary(s.ukprn, s.academicYear, s.period, s.grantType, sum(s.totalPayment)) " +
            "from  uk.gov.london.ops.domain.skills.SkillsFundingSummaryEntity s " +
    ...
    @Query(value = "select count(c.id) from claim c inner join learning_grant_entry lge on c.entity_id = lge.id" +
            " inner join learning_grant_block lgb on c.block_id=lgb.id where grant_type='AEB_PROCURED' and lge.academic_year=?1 and period=?2", nativeQuery = true)
