...
import uk.gov.london.ops.domain.project.InternalRiskBlock;
...
import uk.gov.london.ops.domain.template.QuestionsBlockSection;
...
public class TemplateProjectService extends BaseProjectService {
    ...
    public void addSection(Template template, int blockDisplayOrder, QuestionsBlockSection section) {
        ...
        for (Project project : projects) {
            ...
            List<NamedProjectBlock> lastApprovedAndUnapproved = project.getLastApprovedAndUnapproved(ProjectBlockType.Questions, blockDisplayOrder);

            for (NamedProjectBlock block : lastApprovedAndUnapproved) {
                ProjectQuestionsBlock questionsBlock = (ProjectQuestionsBlock) block;
                QuestionsBlockSection toAdd = new QuestionsBlockSection(section.getExternalId(), section.getDisplayOrder(), section.getText());
    ...
    public void replaceQuestion(Template template, Integer blockDisplayOrder, Integer oldQuestionId, Question newQuestion) {
        List<Project> projects = projectRepository.findAllByTemplate(template);
        for (Project project: projects) {
            List<NamedProjectBlock> blocks = project.getBlocksByTypeAndDisplayOrder(ProjectBlockType.Questions, blockDisplayOrder);
        ...
        for (Project project: projects) {
            for (NamedProjectBlock projectBlock: project.getBlocksByTypeAndDisplayOrder(ProjectBlockType.Milestones, blockDisplayOrder)) {
                ...
                if (milestonesBlock.getProcessingRouteId() == null // projects with default processing route wont store its ID
                    ...
                    if(milestonesBlock.getMilestoneByExternalId(milestoneId) != null) { //null check for projects that haven't selected a processing route yet
    ...
    public void updateAssessmentsAffectedByCategoryChange(Set<Template> templates, String oldName, String newName) {

        for (Template template : templates) {
            Set<OutputCategoryAssumption> allAffectedByNameChange = outputCategoryAssumptionRepository.findAllAffectedByNameChange(template.getId(), oldName);
  ...
  public void updateInfoMessage(Template template, Integer displayOrder, String infoMessage) {
    ...
    public void migrateContactDetails(Template template, Integer questionsBlockDisplayOrder, Integer contactNameQuestionId, Integer contactEmailQuestionId) {
        List<Project> projectsForTemplate = projectRepository.findAllByTemplate(template);
        for (Project project: projectsForTemplate) {
            for (NamedProjectBlock block: project.getBlocksByTypeAndDisplayOrder(ProjectBlockType.Questions, questionsBlockDisplayOrder)) {
                ProjectQuestionsBlock questionsBlock = (ProjectQuestionsBlock) block;
                ...
                if (emailAnswer == null) {
                    emailAnswer = new Answer(questionsBlock.getTemplateQuestionByQuestionId(contactEmailQuestionId).getQuestion());
