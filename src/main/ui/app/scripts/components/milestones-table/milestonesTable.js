...
class MilestonesTable {
  ...
  constructor(MilestonesService, ProjectMilestoneModal, ConfirmationDialog, ClaimMilestoneModal, ReclaimMilestoneModal, UserService, EvidenceModal, ReclaimInfoModal) {
  ...
  refreshData(focus, recentlyAdded) {
    ...
    if (focus) {
      this.MilestonesService.focusNextElement();
    }
  ...
  showEvidenceModal(milestone) {
    if (this.showExtraInfo) {
      this.EvidenceModal.show(this.project, this.projectBlock.id, milestone, this.maxEvidenceAttachments, this.readOnly);
    }
  }
...
MilestonesTable.$inject = ['MilestonesService', 'ProjectMilestoneModal', 'ConfirmationDialog', 'ClaimMilestoneModal', 'ReclaimMilestoneModal', 'UserService', 'EvidenceModal', 'ReclaimInfoModal'];
