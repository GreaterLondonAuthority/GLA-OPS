...
class AnnualSubmissionCtrl {
  ...
  $onInit() {
    ...
    transitionButtons = _.sortBy(transitionButtons, 'dispalyOrder');
  ...
  autoSave() {
    let annualSubmissionRequest = _.pick(this.annualSubmission, ['submissionComments', 'approvalComments', 'dpfRollover', 'rcgfRollover', 'authorisedBy', 'authorisedByJobTitle', 'authorisedOn']);
  ...
  approvalButtonDisabledFn() {
    if (!(this.annualSubmission.approvalComments)) {
      return true
    }
    if (this.hasRcgf &&
