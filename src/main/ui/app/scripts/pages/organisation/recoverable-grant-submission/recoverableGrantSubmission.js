...
class RecoverableGrantSubmissionCtrl {
  ...
  $onInit() {
    ...
    this.tiles =  this.AnnualSubmissionService.getBalanceTiles(this.block);
  }
  ...
  showGrantIncomeModal(entry, isSpent) {
    ...
    let modal = this.GrantIncomeModal.show(entry, title, [{categories: availableCategories}]);
  ...
  showGrantDependantFields(){
    return this.block.totalUnspentGrant > 0 || this.block.interestedAccumulated > 0;
    // return this.block.totalUnspentGrant >= 0 || this.block.interestedAccumulated >= 0;
    // return this.block.totalUnspentGrant != undefined || this.block.interestedAccumulated != undefined;
  }
