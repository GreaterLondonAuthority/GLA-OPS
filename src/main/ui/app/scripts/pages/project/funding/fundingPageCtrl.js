...
class FundingPageCtrl extends ProjectBlockCtrl {
  constructor($injector, $scope, ProjectService, ProjectFundingService, ReferenceDataService, FundingEvidenceModal, orderByFilter) {
    super($injector);
    this.FundingEvidenceModal = FundingEvidenceModal;
  ...
  $onInit() {
    ...
    if(this.projectBlock.categoriesExternalId) {
      this.ReferenceDataService.getConfigItemsByExternalId(this.projectBlock.categoriesExternalId).then(categories => {
  ...
  showEvidenceModal(activity) {
    ...
    let modal = this.FundingEvidenceModal.show(this.project, this.projectBlock.id, activity, this.maxEvidenceAttachments, this.readOnly);
    modal.closed.then(()=>{
  ...
  onClaimQuarter(data){
    return this.ProjectFundingService.claim(this.project.id, this.projectBlock.id, data.year, data.quarter).then(resp => {
      return this.ProjectFundingService.getProjectFunding(this.project.id, this.projectBlock.id, this.blockSessionStorage.selectedYear).then((resp) => {
        return this.refreshPageData(resp.data);
      });
    });
...
FundingPageCtrl.$inject = ['$injector','$scope', 'ProjectService', 'ProjectFundingService', 'ReferenceDataService', 'FundingEvidenceModal','orderByFilter'];
