...
public class UserPasswordService {
    ...
    @Value("${password.expiry.period.in.days}")
    ...
    CreatePasswordResetTokenResponse createPasswordResetToken(User user) {
        ...
        if (!user.isEnabled()) {
            ...
            emailId = emailService.sendDisabledUserPasswordResetAttemptEmail(user);
        } else {
            String token = createRandomPasswordResetToken();
            PasswordResetToken passwordResetToken = storePasswordResetToken(token, user);
            emailId = emailService.sendPasswordResetEmail(user, passwordResetToken.getId(), token);
        }
        return new CreatePasswordResetTokenResponse(emailId);
    }
    ...
    public void specifyUserPassword(String username, String password) {
        if (environment.shortName().equals("PPD") || environment.isTestEnvironment()) {
    ...
    public void setPassword(String username, String password) {
        User user = userService.find(username);
        if (userService.canResetUserPassword(username)) {
            setNewPassword(user, password, true);
        } else {
            auditService.auditCurrentUserActivity(String.format("Unsuccessful attempt to modify password for user %s by user %s", username, userService.currentUsername()));
            throw new ForbiddenAccessException(String.format("Current user %s does not have the suitable roles to reset the password for user %s", userService.currentUsername(), username));
        }
    }
    ...
    public void setNewPassword(User user, String newPassword, boolean audit) {
        ...
        user.setPasswordExpiry(environment.now().plusDays(passwordExpiryPeriod));
    ...
    public void setUserPasswordExpiry(String username, OffsetDateTime passwordExpiry) {
        User user = userService.find(username);
        user.setPasswordExpiry(passwordExpiry);
        userService.save(user);
        auditService.auditCurrentUserActivity("Updated password expiry time for user " + username);
    }
}
